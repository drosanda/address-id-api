...
{
    public $tbl = 'd_kecamatan';
    public $tbl_as = 'dkec';
    public $tbl2 = 'd_kabkota';
    public $tbl2_as = 'dkk';
    ...
    {
        if (strlen($d_kabkota_id)>0 && $d_kabkota_id>0) {
    ...
    public function getSearch($d_kabkota_id, $kabkota_nama, $keyword="")
    {
        ...
        if (strlen($d_kabkota_id)>0 && $d_kabkota_id>0) {
            $this->db->where_as("COALESCE($this->tbl2_as.id)", $this->db->esc($d_kabkota_id), "AND", "LIKE");
        }
        if (strlen($kabkota_nama)>0) {
            $this->db->where_as("COALESCE($this->tbl2_as.nama)", $this->db->esc($kabkota_nama), "AND", "LIKE");
        }
        if (strlen($keyword)>0) {
