...
{
    public $tbl = 'd_kabkota';
    public $tbl_as = 'dk';
    public $tbl2 = 'd_provinsi';
    public $tbl2_as = 'dp';
    ...
    public function get($d_provinsi_id)
    {
        ...
        if (strlen($d_provinsi_id)>0 && $d_provinsi_id>0) {
            $this->db->where("d_provinsi_id", $d_provinsi_id);
        }
    ...
    public function getSearch($d_provinsi_id, $provinsi_nama, $keyword="")
    {
        ...
        if (strlen($d_provinsi_id)>0 && $d_provinsi_id>0) {
            $this->db->where_as("COALESCE($this->tbl2_as.id)", $this->db->esc($d_provinsi_id), "AND", "LIKE");
        }
        if (strlen($provinsi_nama)>0) {
            $this->db->where_as("COALESCE($this->tbl2_as.nama)", $this->db->esc($provinsi_nama), "AND", "LIKE");
        }
        if (strlen($keyword)>0) {
