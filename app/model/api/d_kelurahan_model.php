<?php
class D_Kelurahan_Model extends SENE_Model
{
    public $tbl = 'd_kelurahan';
    public $tbl_as = 'dkel';
    public $tbl2 = 'd_kecamatan';
    public $tbl2_as = 'dkec';
    ...
    public function get($d_kecamatan_id)
    {
        ...
        if (strlen($d_kecamatan_id)>0 && $d_kecamatan_id>0) {
    ...
    public function getSearch($d_kecamatan_id, $kecamatan_nama, $keyword="")
    {
        ...
        $this->db->from($this->tbl, $this->tbl_as);
        $this->db->join($this->tbl2, $this->tbl2_as, 'id', $this->tbl_as, 'd_kecamatan_id', 'left');
        if (strlen($d_kecamatan_id)>0 && $d_kecamatan_id>0) {
            $this->db->where_as("COALESCE($this->tbl2_as.id)", $this->db->esc($d_kecamatan_id), "AND", "LIKE");
        }
        if (strlen($kecamatan_nama)>0) {
            $this->db->where_as("COALESCE($this->tbl2_as.nama)", $this->db->esc($kecamatan_nama), "AND", "LIKE");
        }
        if (strlen($keyword)>0) {
