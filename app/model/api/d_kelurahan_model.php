...
class D_Kelurahan_Model extends SENE_Model{
  ...
  public function getSearch($d_kecamatan_id,$kecamatan_nama,$keyword=""){
    ...
    if(strlen($d_kecamatan_id)>0 && $d_kecamatan_id>0) $this->db->where_as("COALESCE($this->tbl2_as.id)",$this->db->esc($d_kecamatan_id),"AND","LIKE");
    if(strlen($kecamatan_nama)>0) $this->db->where_as("COALESCE($this->tbl2_as.nama)",$this->db->esc($kecamatan_nama),"AND","LIKE");
